...
let BrandsComponent = class BrandsComponent {
    constructor(brandService, tostador, route) {
    ...
    swapStatus(brand) {
        ...
        this.brandService.swapStatus(brand._id, status)
            .subscribe(() => brand.status = status, (err) => this.tostador.error(err.error.message, '¡Error!')).add(() => { });
    }
    findBrand(input) {
...
let EditComponent = class EditComponent {
    constructor(brandsService, route, tostador, router) {
    ...
    ngOnInit() {
        ...
        if (value) {
            this.brandsService.fetch(0, 1, 0, value)
                .subscribe((res) => this.brand = res.data[0], (err) => this.tostador.error(err.error, '¡Error!')).add(() => { });
        }
    }
    save(value) {
        value = value.trim();
        if (value) {
            this.brandsService.edit(this.brand._id, value)
                .subscribe(() => this.router.navigateByUrl('/glass-brands/' + this.curr), (err) => this.tostador.error(err.error, '¡Error!')).add(() => { });
        }
    }
};
...
let NewComponent = class NewComponent {
    constructor(brandService, tostador, router, route) {
        this.brandService = brandService;
        this.tostador = tostador;
        this.router = router;
        this.route = route;
    ...
    save(brand) {
        ...
        if (brand) {
            this.brandService.save(brand)
                .subscribe(() => this.router.navigateByUrl('/glass-brands/' + this.curr), (err) => this.tostador.error(err.error.message, '¡Error!')).add(() => { });
        }
    }
};
