...
var EditComponent = /** @class */ (function () {
    function EditComponent(modelService, brandService, tostador, router, route) {
    ...
    EditComponent.prototype.ngOnInit = function () {
        ...
        if (value) {
            this.modelService.fetch(0, 1, value)
                .subscribe(function (res) { return _this.model = res.data[0]; }, function (err) { return _this.tostador.error(err.error, '¡Error!'); }).add(function () { _this.getBrands(); });
        }
    };
    EditComponent.prototype.getBrands = function () {
        var _this = this;
        this.brandService.fetch(0, 10000, 1)
            .subscribe(function (res) {
            _this.brands = res.data;
            _this.selectedOption = _this.model.brand._id;
        }, function (err) { return _this.tostador.error(err.error.message, '¡Error!'); }).add(function () { });
    };
    EditComponent.prototype.save = function (model) {
        var _this = this;
        var brandi = this.selectedOption;
        model = model.trim();
        if (model && brandi) {
            this.modelService.edit(this.model._id, model, brandi)
                .subscribe(function () { return _this.router.navigateByUrl('/glass-models/' + _this.curr); }, function (err) { return _this.tostador.error(err.error.message, '¡Error!'); }).add(function () { });
        }
    };
...
var ModelsComponent = /** @class */ (function () {
    function ModelsComponent(modelService, tostador, route) {
        this.modelService = modelService;
        this.tostador = tostador;
        this.route = route;
        this.models = [];
    ...
    ModelsComponent.prototype.fetchModels = function (name) {
        var _this = this;
        if (!isNaN(this.curr)) {
            this.pages = 0;
            this.cells = [];
            this.modelService.fetch(this.curr, 10, name)
                ...
                _this.showing = _this.models.length < 10 ? _this.count : (_this.models.length * _this.curr);
            }, function (err) { return _this.tostador.error(err.error.message, '¡Error!'); }).add(function () { });
        }
    };
    ModelsComponent.prototype.swapStatus = function (model) {
        var _this = this;
        var status = model.status === 'active' ? 'inactive' : 'active';
        this.modelService.swapStatus(model._id, status)
            .subscribe(function () { return model.status = status; }, function (err) { return _this.tostador.error(err.error.message, '¡Error!'); }).add(function () { });
    };
    ModelsComponent.prototype.findModel = function (input) {
        var _this = this;
        ...
        this.timer = setTimeout(function () {
            ...
            if (input.length === 0) {
                ...
                _this.fetchModels(input);
            }
        }, 500);
    };
    ModelsComponent.prototype.clearTime = function () {
        clearTimeout(this.timer);
    };
...
var NewComponent = /** @class */ (function () {
    function NewComponent(modelService, brandService, tostador, router, route) {
        this.modelService = modelService;
        this.brandService = brandService;
        this.tostador = tostador;
        this.router = router;
    ...
    NewComponent.prototype.ngOnInit = function () {
        ...
        this.curr = Number(this.route.snapshot.params.page) || 1;
        this.brandService.fetch(0, 10000, 1)
            .subscribe(function (res) { return _this.brands = res.data; }, function (err) { return _this.tostador.error(err.error.message, '¡Error!'); }).add(function () { });
    };
    NewComponent.prototype.save = function (model, brandi) {
        var _this = this;
        model = model.trim();
        if (model && brandi) {
...
//# sourceMappingURL=bmodels-models-module-es5.js.map