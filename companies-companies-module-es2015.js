...
let CompaniesRoutingModule = class CompaniesRoutingModule {
};
...
let CompaniesComponent = class CompaniesComponent {
    constructor(route, COMPANY_SERVICE, TOSTADOR) {
        this.route = route;
...
let EditComponent = class EditComponent {
    constructor(ROUTE, ROUTER, COMPANY_SERVICE, TOSTADOR, ME, IMG_HANDLER) {
        this.ROUTE = ROUTE;
        this.ROUTER = ROUTER;
        this.COMPANY_SERVICE = COMPANY_SERVICE;
        this.TOSTADOR = TOSTADOR;
    ...
    ngOnInit() {
        ...
        if (name) {
            ...
            }, (err) => this.TOSTADOR.error(err.message, '¡Error!')).add(() => { this.createForm(); this.me = this.ME.getData(); });
        }
    }
    createForm() {
    ...
    save() {
        if (this.form.status === 'VALID') {
            ...
            this.COMPANY_SERVICE.edit(this.company._id, fd)
                .subscribe(() => this.ROUTER.navigateByUrl('/companies/' + this.curr), (err) => this.TOSTADOR.error(err.message, '¡Error!')).add(() => { });
        }
    }
    plusComments() {
...
let NewComponent = class NewComponent {
    constructor(tostador, router, route, imgHandler, cLs, comService) {
        this.tostador = tostador;
        this.router = router;
        this.route = route;
