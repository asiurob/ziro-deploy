...
let MeRoutingModule = class MeRoutingModule {
};
...
/* harmony import */ var _variables_services_config__WEBPACK_IMPORTED_MODULE_8__ = __webpack_require__(/*! ../../../variables/services.config */ "./src/app/variables/services.config.ts");
...
let MeComponent = class MeComponent {
    constructor(tostador, cIHandler, cLs, cUserService, router) {
    ...
    ngOnInit() {
        ...
        if (name) {
            this.cUserService.fetch(0, 1, name)
                ...
                this.user.photo = `${_variables_services_config__WEBPACK_IMPORTED_MODULE_8__["link"]}/users/${this.user.photo}`;
            ...
            }, (err) => this.tostador.error(err.error, '¡Error!')).add(() => this.createForm());
        }
    }
    createForm() {
    ...
    save() {
        if (this.form.status === 'VALID') {
            ...
            }, (err) => this.tostador.error(err.error, '¡Error!')).add(() => { });
        }
    }
    renderImage(file) {
